...
tickers = ['BRK-B',
           'CI','UNH',
           'AAPL','GOOGL', 'MSFT','ADBE','AMZN','CSCO','INTU','TSLA','META','ORCL','CRM','NFLX'
           ,'AMD','NVDA',] 
...
for ticker in tickers:
    ...
    six_month_intervals = pd.date_range(start=start_date, end=end_date, freq='6M')

    if six_month_intervals[-1] != end_date:
        six_month_intervals = six_month_intervals.append(pd.to_datetime([end_date]))
    ...
    for i in range(len(six_month_intervals) - 1):
        ...
        df_interval.to_csv(f'{ticker_dir}/{ticker}_{start_interval.date()}_{end_interval.date()}.csv')
